			  ...
			  struct am65_cpts_estf_req *req)
{
	...
	if (req->on) {
		cycles = req->ns_period * cpts->refclk_freq;
		cycles = DIV_ROUND_UP(cycles, NSEC_PER_SEC);
		if (cycles > U32_MAX)
			return -EINVAL;

		/* according to TRM should be zeroed */
		am65_cpts_write32(cpts, 0, estf[req->idx].length);

		val = upper_32_bits(req->ns_start);
		am65_cpts_write32(cpts, val, estf[req->idx].comp_hi);
		val = lower_32_bits(req->ns_start);
		am65_cpts_write32(cpts, val, estf[req->idx].comp_lo);
		val = lower_32_bits(cycles);
		am65_cpts_write32(cpts, val, estf[req->idx].length);
	} else {
		am65_cpts_write32(cpts, 0, estf[req->idx].length);
	}

	dev_dbg(cpts->dev, "%s: ESTF:%u %s\n", __func__, req->idx,
		req->on ? "enabled" : "disabled");
